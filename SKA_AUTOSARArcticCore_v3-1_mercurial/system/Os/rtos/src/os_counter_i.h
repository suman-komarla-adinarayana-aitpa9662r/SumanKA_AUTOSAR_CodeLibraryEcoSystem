...
typedef struct OsCounter {
    ...
    TickType val;			/* The counter value ( if software counter ) */
...
static inline TickType Os_CounterDiff( TickType curr, TickType old, TickType max ) {
    /* + 1 here because there is one tick between OSMAXALLOWEDVALUE and 0.
     * That is, if curr = 0, max = 9 and old = 0, then the diff should be 10.
     */
...
static inline TickType Os_CounterAdd( TickType curr, TickType max, TickType add ) {
    ...
    if( add <= diff  ) {
        ...
        /* @req SWS_Os_00384
         * The Operating System module shall adjust the read out values of hardware timers (which drive counters)
         * in such that the lowest value is zero and consecutive reads return an increasing count value until
